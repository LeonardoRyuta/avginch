...
/**
 * @title ICP Source Escrow for EVM→icp atomic swaps
 * @notice Escrow contract for EVM→ICP swaps - holds ERC20/ETH, releases when taker provides secret
 * @dev Used when EVM tokens are the source and icp is the destination
 * @custom:security-contact security@atomicswap.io
 */
contract ICPEscrowSrc is Escrow {
    using SafeERC20 for IERC20;
    using AddressLib for Address;
    using TimelocksLib for Timelocks;

    /// @notice icp address where funds should be sent
    mapping(bytes32 => string) public icpAddresses;
    
    /// @notice icp transaction hash for verification (optional)
    mapping(bytes32 => string) public icpTxHashes;
     ...
     * @dev Taker reveals secret to claim EVM tokens after providing icp to maker
    ...
    /**
     * @notice Records icp address for the swap
     * @dev Links icp address to escrow for verification
     * @param hashlock The escrow hashlock
     * @param btcAddress The icp address where funds should be sent
     * @param immutables The escrow immutables
     */
    function recordicpAddress(
        ...
        onlyValidImmutables(immutables)
    {
        // Only maker can record icp address
    ...
    function recordicpTx(
        ...
        onlyValidImmutables(immutables)
    {
        // Only taker can record icp tx (proof of payment)
    ...
    /**
     * @notice Gets recorded icp address
     * @param hashlock The escrow hashlock
     * @return The icp address
     */
    function geticpAddress(bytes32 hashlock) external view returns (string memory) {
        return icpAddresses[hashlock];
    }

    /**
     * @notice Gets recorded icp transaction hash
     * @param hashlock The escrow hashlock
     * @return The icp transaction hash
     */
    function geticpTxHash(bytes32 hashlock) external view returns (string memory) {
        return icpTxHashes[hashlock];
    }
