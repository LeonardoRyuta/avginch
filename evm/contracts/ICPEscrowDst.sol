...
/**
 * @title ICP Destination Escrow for icp→EVM atomic swaps
 * @notice Escrow contract for ICP→EVM swaps - holds ERC20/ETH, releases on secret reveal
 * @dev Used when icp is the source and EVM tokens are the destination
 * @custom:security-contact security@atomicswap.io
 */
contract ICPEscrowDst is Escrow {
     ...
     * @dev Maker reveals secret to claim EVM tokens after providing icp
    ...
    function recordicpTx(
        ...
        onlyValidImmutables(immutables)
    {
        // Only maker or taker can record icp tx
        if (msg.sender != immutables.maker.get() && msg.sender != immutables.taker.get()) {
            revert InvalidCaller();
        }
    ...
    function recordicpAddress(
        ...
        onlyValidImmutables(immutables)
    {
        // Only maker or taker can record icp address
        if (msg.sender != immutables.maker.get() && msg.sender != immutables.taker.get()) {
            revert InvalidCaller();
        }
